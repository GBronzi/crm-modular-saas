...
import { loginSchema,passwordResetConfirmSchema,passwordResetRequestSchema,provisionSchema } from '../src/auth/auth.schemas.js';
describe('auth schemas',()=>{
  ...
  it('validates password reset payloads',()=>{ expect(passwordResetRequestSchema.parse({companySlug:'ACME',email:' ADMIN@EXAMPLE.COM '}).email).toBe('admin@example.com'); expect(passwordResetConfirmSchema.safeParse({companySlug:'acme',token:'x'.repeat(32),password:'a-secure-password'}).success).toBe(true); });
});
